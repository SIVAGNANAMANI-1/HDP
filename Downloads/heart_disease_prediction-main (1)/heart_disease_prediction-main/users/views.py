...
def send_otp_sms(phone_number, otp):
    """Send OTP via SMS using Twilio. Returns True on success, False on failure."""
    try:
        ...
        if account_sid == 'your_account_sid' or auth_token == 'your_auth_token':
            logger.warning("Twilio is not configured. Skipping SMS OTP.")
            return False
...
def home(request):
    # DISABLED: Authentication/role check bypassed
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if email_sent or sms_sent:
                ...
                if sms_sent:
                    delivery_methods.append('phone')
                messages.success(request, f"OTP sent to your {' and '.join(delivery_methods)}. Please verify.")
            else:
                messages.warning(request, "Could not send OTP. Please try resending.")
...
def resend_otp(request):
    ...
    if email_sent or sms_sent:
        ...
        if sms_sent:
            delivery_methods.append('phone')
        messages.success(request, f"OTP resent to your {' and '.join(delivery_methods)}.")
    else:
        messages.error(request, "Failed to resend OTP. Please check your email/phone or try again.")
...
def login_view(request):
    # DISABLED: Authentication bypassed — redirects straight to dashboard
    if request.method == "POST":
        ...
        return redirect('doctor_dashboard')  # Skip authentication
...
# @login_required  # DISABLED: Authentication bypassed
def doctor_dashboard(request):
    # Any context or logic related to the doctor dashboard can be added here
    doctor_name = request.user.username if request.user.is_authenticated else 'Doctor'  # Fallback when not logged in
